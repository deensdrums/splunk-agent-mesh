...
class DevSettingsStore(SettingsStore):
    ...
    def store_api_key(self, key: str) -> None:
        ...
        if not DEV_MODE:
            logger.error(
                "Refused to store API key in plaintext. Set AGENT_MESH_DEV_MODE=1 to enable "
                "local key persistence, or set AGENT_MESH_API_KEY env var."
            )
            raise PermissionError(
                "Plaintext key storage refused. Set AGENT_MESH_DEV_MODE=1 or use AGENT_MESH_API_KEY env var."
            )
        existing: dict = {}
        if _LOCAL_SETTINGS_PATH.exists():
            try:
...
class SplunkSecureSettingsStore(SettingsStore):
    ...
    def get_provider_settings(self) -> dict:
        # TODO: read from /services/properties/agent_mesh/settings
        raise NotImplementedError("SplunkSecureSettingsStore.get_provider_settings not yet wired.")

    def save_provider_settings(self, provider: str, base_url: str | None, model: str) -> None:
        # TODO: POST to /services/configs/conf-agent_mesh
    ...
    def get_api_key(self) -> str | None:
        # TODO: GET /services/storage/passwords/agent_mesh:llm_api_key:
        raise NotImplementedError("SplunkSecureSettingsStore.get_api_key not yet wired.")

    def clear_api_key(self) -> None:
        # TODO: DELETE /services/storage/passwords/agent_mesh:llm_api_key:
